...
#ifndef _huffman_encoding_Aem_H_
#define _huffman_encoding_Aem_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "huffman_encoding_Aem_memcore.h"

SC_MODULE(huffman_encoding_Aem) {
...
protected:
    ...
    huffman_encoding_Aem_memcore* huffman_encoding_Aem_memcore_U;
...
public:
    ...
    SC_CTOR(huffman_encoding_Aem) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        huffman_encoding_Aem_memcore_U = new huffman_encoding_Aem_memcore("huffman_encoding_Aem_memcore_U");
        huffman_encoding_Aem_memcore_U->clk      ( clk );
        huffman_encoding_Aem_memcore_U->reset    ( reset );
        huffman_encoding_Aem_memcore_U->ce0      ( i_ce0 );
        huffman_encoding_Aem_memcore_U->we0      ( i_we0 );
        huffman_encoding_Aem_memcore_U->address0 ( memcore_iaddr );
        huffman_encoding_Aem_memcore_U->d0       ( i_d0 );
        huffman_encoding_Aem_memcore_U->q0       ( i_q0 );

        huffman_encoding_Aem_memcore_U->ce1      ( t_ce0 );
        huffman_encoding_Aem_memcore_U->we1      ( t_we0 );
        huffman_encoding_Aem_memcore_U->address1 ( memcore_taddr );
        huffman_encoding_Aem_memcore_U->d1       ( t_d0 );
        huffman_encoding_Aem_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "huffman_encoding_Aem.trace");
            // ports
