...
void huffman_encoding_sc4::proc_i_q0() {
    i_q0.write(buf_q0[prev_iptr.read()].read());
}

void huffman_encoding_sc4::proc_t_q0() {
    t_q0.write(buf_q0[prev_tptr.read()].read());
}

void huffman_encoding_sc4::proc_i_q1() {
    i_q1.write(buf_q1[prev_iptr.read()].read());
}

void huffman_encoding_sc4::proc_t_q1() {
    t_q1.write(buf_q1[prev_tptr.read()].read());
}

void huffman_encoding_sc4::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
