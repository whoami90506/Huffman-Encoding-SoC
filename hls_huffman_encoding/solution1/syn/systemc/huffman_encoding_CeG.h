...
#ifndef _huffman_encoding_CeG_H_
#define _huffman_encoding_CeG_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "huffman_encoding_CeG_memcore.h"

SC_MODULE(huffman_encoding_CeG) {
...
protected:
    ...
    // buffer instances
    huffman_encoding_CeG_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(huffman_encoding_CeG) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "huffman_encoding_CeG_memcore_U_";
            name += szTmp;

            buffer[i] = new huffman_encoding_CeG_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "huffman_encoding_CeG.trace");
            // ports
