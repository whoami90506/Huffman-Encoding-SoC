...
#ifndef _huffman_encoding_rcU_H_
#define _huffman_encoding_rcU_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "huffman_encoding_rcU_memcore.h"

SC_MODULE(huffman_encoding_rcU) {
...
protected:
    ...
    // buffer instances
    huffman_encoding_rcU_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(huffman_encoding_rcU) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "huffman_encoding_rcU_memcore_U_";
            name += szTmp;

            buffer[i] = new huffman_encoding_rcU_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "huffman_encoding_rcU.trace");
            // ports
