    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUM_NONZERO_SYMBOLS_DATA_0: begin
                    rdata <= int_num_nonzero_symbols[31:0];
                end
                ADDR_NUM_NONZERO_SYMBOLS_CTRL: begin
...
// int_num_nonzero_symbols
always @(posedge ACLK) begin
    if (ARESET)
        int_num_nonzero_symbols <= 0;
    else if (ACLK_EN) begin
        if (num_nonzero_symbols_ap_vld)
            int_num_nonzero_symbols <= num_nonzero_symbols;
    end
end

// int_num_nonzero_symbols_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_num_nonzero_symbols_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (num_nonzero_symbols_ap_vld)
            int_num_nonzero_symbols_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_NUM_NONZERO_SYMBOLS_CTRL)
            int_num_nonzero_symbols_ap_vld <= 1'b0; // clear on read
    end
end
