        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUM_NONZERO_SYMBOLS_DATA_0 =>
                        rdata_data <= RESIZE(int_num_nonzero_symbols(31 downto 0), 32);
                    when ADDR_NUM_NONZERO_SYMBOLS_CTRL =>
                        rdata_data <= (0 => int_num_nonzero_symbols_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_num_nonzero_symbols <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (num_nonzero_symbols_ap_vld = '1') then
                    int_num_nonzero_symbols <= UNSIGNED(num_nonzero_symbols); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_num_nonzero_symbols_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (num_nonzero_symbols_ap_vld = '1') then
                    int_num_nonzero_symbols_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_NUM_NONZERO_SYMBOLS_CTRL) then
                    int_num_nonzero_symbols_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
